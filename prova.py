        ...
        for i in range(dimHoriz):
            ...
            for z in range(len(blockHistogramVector)):
                for w in range(nbins):
                    if sumHistogram[w] != 0:
                        histogramsNormalized.append(blockHistogramVector[z][w] / sumHistogram[w])
                    else:
                        histogramsNormalized.append(0.)
...
# test_mit = glob('MIT/*')
test_prova_pos = glob('prova/aux/*')
test_prova_neg = glob('prova/neg/*')

for fn in it.chain(test_prova_pos):
    try:
...
for fn in it.chain(test_prova_neg):
    try:
        # Retornem imatge en escala de grisos normalitzada
        img = load_image(fn)
